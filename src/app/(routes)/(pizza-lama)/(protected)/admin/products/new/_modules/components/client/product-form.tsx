...
import { ProductFormProps } from "../client";
...
export default function ProductForm({ categories, sizes }: ProductFormProps) {
  ...
  const toastLoading = "Creating product... Please wait.";
  ...
  const getAvailableSizes = (currentIndex: number) => {
    const selectedSizeIds = fields
      .map((field, index) => index !== currentIndex ? form.watch(`sizes.${index}.sizeId`) : null)
      .filter(Boolean);
    
    return sizes.filter(size => !selectedSizeIds.includes(size.id));
  };
  const allSelectedSizeIds = fields.map((_, i) => form.watch(`sizes.${i}.sizeId`)).filter(Boolean);
  const remainingSizes = sizes.filter(size => !allSelectedSizeIds.includes(size.id));
